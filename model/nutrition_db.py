...
# ──────────────────────────────────────────────
def load(path: str = INDEX_PATH) -> bool:
    """Load the nutrition index from disk. Returns True if successful."""
    global _db, _loaded
    if _loaded:
        return True
    if not os.path.exists(path):
...
def fuzzy_search(query: str, top_n: int = 5) -> List[dict]:
    ...
    matches = difflib.get_close_matches(query.lower().strip(), keys, n=top_n, cutoff=0.45)
    return [foods[m] for m in matches]


def search_by_keyword(keyword: str, top_n: int = 10) -> list[dict]:
    """Substring search across all food names."""
    foods = _db.get("foods", {})
    kw = keyword.lower().strip()
    results = [rec for key, rec in foods.items() if kw in key]
...
def get_protocol_foods(protocol: str, top_n: int = 10) -> list[dict]:
    """
    Return top_n foods for a given protocol/state.
    Protocol keys: 'stress', 'energy', 'sleep', 'fat_loss', 'muscle_gain',
                   'high_protein', 'low_calorie', 'magnesium', 'iron', 'tryptophan'
    """
    tag = PROTOCOL_TAGS.get(protocol.lower(), protocol.lower() + "_protocol")
    tag_index = _db.get("tag_index", {})
    food_names = tag_index.get(tag, [])[:top_n]
    foods = _db.get("foods", {})
    return [foods[n.lower()] for n in food_names if n.lower() in foods]
...
# ──────────────────────────────────────────────
def _format_food(rec: dict, detail: bool = False) -> str:
    """One-line summary of a food record."""
    name = rec.get("name", "Unknown")
    cal  = rec.get("calories", "?")
    prot = rec.get("protein_g", 0)
    carbs = rec.get("carbs_g", 0)
    fat  = rec.get("fat_g", 0)
    fiber = rec.get("fiber_g", 0)
    base = f"{name}: {cal} kcal | P:{prot}g C:{carbs}g F:{fat}g"
    if detail:
        tags = rec.get("tags", [])
        micros = []
        for field, label in [("magnesium_mg", "Mg"), ("iron_mg", "Fe"),
                              ("tryptophan_mg", "Trp"), ("vitamin_b12_ug", "B12")]:
            v = rec.get(field)
            if v:
                micros.append(f"{label}:{v}")
        if fiber:
...
def build_nutrition_context(profile: dict) -> str:
    """
    Build a compact nutrition knowledge block to inject into the AI context.
    Selects relevant foods based on the user's risk profile and goals.
    """
    if not _loaded:
        return ""
    ...
    lines = [
        "\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"  NUTRITION DATABASE  ({meta().get('total_foods', '?')} foods indexed)",
        "  Source: Kaggle — Nutritional Values for Common Foods",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
    ]
    ...
    # Always include goal-specific foods
    if "fat loss" in goal or "weight loss" in goal:
        sections.append(("🎯 Fat Loss Foods", "fat_loss"))
    elif "muscle" in goal or "bulk" in goal:
        sections.append(("🎯 Muscle Gain Foods", "muscle_gain"))
    else:
        sections.append(("🎯 High-Protein Options", "high_protein"))

    # State-based sections
    if stress >= 7:
        sections.append(("😰 Stress Relief Foods (Magnesium-rich)", "stress"))
    if energy <= 4:
        sections.append(("⚡ Energy Restoration Foods (Iron/B12)", "energy"))
    if sleep_q == "poor":
        sections.append(("😴 Sleep Support Foods (Tryptophan-rich)", "sleep"))

    # Always add a high-fiber / gut health section
    sections.append(("🌿 High-Fiber / Gut Health", "high_fiber"))

    for section_title, protocol in sections:
        foods = get_protocol_foods(protocol, top_n=8)
        if not foods:
            continue
        # Filter out non-vegetarian for vegan/vegetarian (basic heuristic)
        if "vegan" in diet or "vegetarian" in diet:
            meat_keywords = ["chicken", "beef", "pork", "turkey", "salmon",
                             "tuna", "fish", "lamb", "bacon", "shrimp", "crab"]
            foods = [f for f in foods
                     if not any(kw in f.get("name", "").lower() for kw in meat_keywords)]
        lines.append(f"\n  {section_title}:")
        for rec in foods[:6]:
            lines.append(f"    • {_format_food(rec, detail=True)}")

    lines.append("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    lines.append("  Use the above real nutritional data when building meal plans.")
    lines.append("  Reference specific foods by name and their actual macros/micros.")
    lines.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
